...
{
	m->img_map=IMG_Load(chemin);//minimap
...
{
	SDL_Rect pos_joueur;
	pos_joueur.x= 400+((camera.x +Posjoueur.x)*redemensionement)/100;
	pos_joueur.y=  -15 + ((camera.y +Posjoueur.y)*redemensionement)/100;
...
void afficherminimap (minimap m,SDL_Surface *screen)
{
	...
	SDL_BlitSurface(m.img_es,NULL,screen,&m.pos_es);
}
